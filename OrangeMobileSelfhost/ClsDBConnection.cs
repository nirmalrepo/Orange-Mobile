...
namespace OrangeMobileSelfhost
{
    ...
    {
        private static ConnectionStringSettings ConnectionStringSettings = ConfigurationManager.ConnectionStrings["OnlineShoppingDatabase"];
        ...
        { // For most DBMS using @Name1, @Name2, @Name3 etc.
            if (prPars != null)
                foreach (KeyValuePair<string, Object> lcItem in prPars)
                {
                    ...
                    lcPar.Value = lcItem.Value == null ? DBNull.Value : lcItem.Value; lcPar.ParameterName = '@' + lcItem.Key;
